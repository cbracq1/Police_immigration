...
```{r}
cprop(svytable(~ agenq_cat + d_lieudisagr_d_rec, indiv_immig_pd))
...
```
```{r}
cprop(svytable(~ sexee_rec + d_lieudisagr_d_rec, indiv_immig_pd))
...
indiv_immigre_brad <- indiv_immigre %>% 
  ...
  filter(i_contri_rec == "Jamais") #1611 qui représentent en pondéré 1036069
...
indiv_immigre_nonbrad <- indiv_immigre %>% 
  ...
  filter(agenq_cat %in% c("1", "2")) %>%  # Avant 40 ans
  filter(sexee_rec=="Masculin") %>% 
  filter(i_contri_rec != "Jamais") #406 qui représentent en pondéré 225366
