...
for (i in 1:5){ #Pour chaque groupe : traçons une courbe confiance envers la police ~ durée de sejour
  ...
  Y = c()
  for (x in X){
    n_tot = length(which(data$variables$durar2==x & data$variables$i_cnfpol_rec != "Refus ou ne sait pas"))
...
Concernant la sécu : se sentir en sécurité favorise la confiance (légère corrélation)
...
-> i_contri : fqce des controle par la police
-> i_controla & i_controlb : en une seule variable i_control_rec (_a,_b,...,_g,_flag)
...
```
On remarque les G2 sont plus touchés par les contôles policiers que les G1 (pour les I et les OM) : ça pourrait être intéressant de regarder selon l'âge.

Nous pourrions déjà trouver une première explication à notre apparent paradoxe : les G2 (certainement plus jeunes) sont plus controlés que les G1 (certainement plus vieux). Ceci ajoute une explication à la différence G1-G2.

De plus nous remarquons que la case "Autre" (= ni immigrés ni descendants d'immigrés) sont plus controlé que les G1 par la police.
...
Enfin, est-ce que vivre en QPV est corrélé avec la frequence des controles de police ?

Nous constatons qu'il y a significativement plus de controle en vivant en QPV que non. Mais surtout plus de controle en n'étant pas G1 !

En ajoutant la catégorie G2 : on observe que les G2 sont plus controlé que les Autres qui le sont plus que les G1.
