...
ok: 
i_cnfpol, i_cnfjus, i_ecole, i_contri, génération et origine (origine_tous_g2), sexe (sexee), group1, sécurité du quartier(l_quart_secu),QPV (qpv_i), présence d'immigrés dans le quartier (l_immi), relations dans le quartier (a_rquart), f_dip plus haut diplôme

à faire :
, indicateurs niveau socioéco (csnq_ego, csnq_premp premier emploi, uc_reve : Montant mensuel des ressources du ménage par unité de consommation en euros, )
...
Modalité : 
soit on veut créer une variable synthétique (genre toutes les personnes qui ont dit oui à au moins un truc négatif ou qui ont au moins pas coché un truc positif)
