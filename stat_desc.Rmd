...
```
Nous retrouvons chez les descendants une forte corrélation entre sécurité du quartier en confiance enl la police : la police serait à l'origine de cette insécurité par présence ou incompétence. Nous confirmerons avec la troisième partie ces hypothèses.
...
# Discrimination policier ou non (pop = metro)
...
# Discrimination policier ou non (pop = immigrés)

tb1 <- data.frame(cprop(svytable(~i_cnfpol_rec+d_lieudisagr_d_rec, indiv_immig[which(indiv_immig$variables$d_lieudisagr_d_rec != "Refus ou ne sait pas")])))
tb2 <- filter(tb1,i_cnfpol_rec != "Total")
...
# Discrimination policier ou non (pop = descendants)

tb1 <- data.frame(cprop(svytable(~i_cnfpol_rec+d_lieudisagr_d_rec, indiv_desce[which(indiv_desce$variables$d_lieudisagr_d_rec != "Refus ou ne sait pas")])))
tb2 <- filter(tb1,i_cnfpol_rec != "Total")
