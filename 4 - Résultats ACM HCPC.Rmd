...
indiv_acm2 <- indiv %>%
  ...
  select(i_cnfpol_rec, i_contri_rec, d_lieudisagr_d_rec, origine_tous_g2_rec, l_immi_rec, a_rquart_code, l_quart_secu_rec, qpv_i_rec, sexee_rec, csnq_ego_rec, f_dip, x_moifr, compor_discri, durtotfrm, durar2, agenq, ident, poidsi)  
...
```{r, include =FALSE}
res <- MCA(indiv_acm2, quali.sup = 4:13, quanti.sup = 14:16, row.w = indiv_acm2_poids$poidsi)
...
plot.HCPC(res.HCPC, choice = 'map', draw.tree = FALSE, title = 'Plan factoriel', label = 'none')
...
clust <- res.HCPC$data.clust[,17] # on ne conserve que la colonne contenant les classes

acm2_classe <- cbind(indiv_acm2_poids, clust)
```

```{r}
xtable(table(acm2_classe$clust))
```


```{r, fig.height=12, echo = FALSE}
acm2_classe$f_dip <- as.factor(acm2_classe$f_dip)
acm2_classe$x_moifr <- as.factor(acm2_classe$x_moifr)

library(GGally)
ggtable(
  acm2_classe, 
  columnsX = "clust", 
  columnsY = c("i_cnfpol_rec", "i_contri_rec", "d_lieudisagr_d_rec", "compor_discri","origine_tous_g2_rec", "qpv_i_rec", "l_quart_secu_rec", "csnq_ego_rec", "l_immi_rec", "sexee_rec", "durar2", "agenq", "f_dip", "x_moifr"),
...
```{r}
fviz_cluster(res.HCPC, addEllipses = TRUE, ellipse.type = "norm")
```
