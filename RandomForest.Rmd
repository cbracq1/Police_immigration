...
#traitement = which(base$x_moifr < 8 & f_dis_flag > -1 & base$pcs_menage != "NSP/Refus/Incodable" & base$x_moifr < 8 & base$x_apparf < 8 & base$p_statsoc < 98 & base$a_avis < 8 & base$dipdet < 99 & base$compor_discri != "Refus ou ne sait pas")
...
indiv_tree <- indiv_tree %>% 
  mutate(secu_quartier = case_when(l_quart_secu_rec == "Bonne" ~ 1,
                                   l_quart_secu_rec == "Moyenne" ~ 2,
                                   l_quart_secu_rec == "Mauvaise" ~ 3))
...
fitree <- rpart(target_tree~., data = data_tree, method = 'class', weights = indiv_tree_i$poidsi, control = rpart.control(cp = 0.005))
#rpart.plot(fitree, extra = 106)
